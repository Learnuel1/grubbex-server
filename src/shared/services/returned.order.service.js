...
exports.create = async (info) => {
    try {   
        const createOrder = await ReturnedOrderModel.create({...info, $set:{
...
exports.updateOrderDetails = async (info, reference) => {
    try{
        ...
        return await ReturnedOrderModel.findOneAndUpdate({reference},{ $set:{...rest}, $push:{
...
exports.orderById = async (orderId ) => {
    try {
        return await ReturnedOrderModel.find( {orderId}).populate([{
            model: "Account",
            path:"destinationAddress.account",
            select: "firstName lastName email  picture -_id"
...
exports.orderByIdForAuth = async (orderId ) => {
    try {
        return await ReturnedOrderModel.findOne({orderId}).populate([{
            model: "Account",
            path:"destinationAddress.account",
            select: "firstName lastName email picture -_id"
